...
``` 
output "list_vm" {
  value = [for server in  yandex_compute_instance.*:{fqdn = server.fqdn, id = server.id, name = server.name }]
}
```
Cкриншот вывода команды ```terrafrom output```.
![Alt text](https://github.com/LeonidKhoroshev/ter-homeworks/blob/main/03/src/screenshots/ter33.png)
